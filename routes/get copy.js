...
const { User, BubbleTransaction, Giveaway } = require('../models'); // ✅ Added Giveaway
const { literal, Op } = require('sequelize');
const sequelize = require('../config/database'); // ✅ Added this line
...
//         const slotLabel = queueSlots > 1 ? ` [Slot ${slotNumber}/${queueSlots}]` : '';
//         const ownLabel = isOwnCard ? ' 👤 YOU' : '';
//         const description = `Queue #${currentQueuePosition}${slotLabel}${ownLabel} • ${currentSlotProgress}/${requiredBubbles} (${queueProgress}%) • ${locationDisplay}`;
...
//           description: `Queue #${slotNumber} • ${slotProgressValue}/${requiredBubbles} (${Math.round((slotProgressValue / requiredBubbles) * 100)}%) • ${currentUser.area || currentUser.city || 'Unknown'}`,
//         });
//       }
//     }
...
router.get('/supporters/:userId', async (req, res) => {
  try {
    ...
    console.log('Backend - Getting supporters for user:', userId, 'Slot:', slotNumber);
  ...
  try {
    ...
    if (queueSlotsToOpen > 0) {
      ...
      if (fromUser.queuePosition === 0) {
        ...
        fromUser.slotProgress = JSON.stringify(supporterSlotProgress); // ← JSON.stringify
...
router.post('/giveaway/donate', async (req, res) => {
  ...
  console.log('🎁 Giveaway donation request:', { userId, category, bubbles });
  ...
  try {
    ...
    while (remaining > 0 && eligibleUsers.length > 0) {
      ...
      console.log(`   🎯 Round ${round} → ${user.name} +${giveAmount}, Remaining: ${remaining}`);
    ...
    console.log(`✅ COMPLETE - Distributed ${totalDistributed} bubbles across ${recipientMap.size} users`);

    // ✅ Fetch updated donor profile to return to frontend
