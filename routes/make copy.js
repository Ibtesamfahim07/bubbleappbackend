// routes/make.js - FIXED VERSION with correct aliases and NO PHONE FIELD
const express = require('express');
const auth = require('../middleware/auth');
const { Brand, Offer, OfferRequest, User } = require('../models/index');
...
// Public endpoint - Get all brands - FIXED ALIAS
  ...
  try {
    const brands = await Brand.findAll({
      include: [{
        ...
        attributes: ['id', 'title', 'description', 'discount', 'type','price'],
        limit: 3
      }],
...
// Public endpoint - Get offers by category
  ...
  try {
    ...
    const offers = await Offer.findAll({
      ...
      include: [{
        model: Brand,
        as: 'Brand',
        attributes: ['id', 'name', 'rating', 'distance', 'featured', 'category','price']
      }],
      order: orderClause,
      limit: 20
    });
...
// Admin endpoint - Get all offer requests - FIXED: REMOVED PHONE FIELD
router.get('/admin/all-offers', auth, async (req, res) => {
  try {
    ...
    console.log('Fetching admin offers with filters:', whereClause);
      ...
      include: [
        {
          model: User,
          as: 'User',
          attributes: ['id', 'name', 'email'] // FIXED: Removed 'phone' - doesn't exist
        },
        {
          model: Offer,
          as: 'Offer',
    ...
    console.log(`Found ${offers.length} offer requests`);
    res.json(offers);
  } catch (error) {
...
// Admin endpoint - Update offer status
...
// Admin endpoint - Get offer statistics
...
// Public endpoint - Search offers and brands
...
// Get user's available giveaway bubbles by category
router.get('/available-giveaway-bubbles/:category', auth, async (req, res) => {
  try {
    ...
    // Map offer categories to giveaway distribution descriptions
    const categoryMap = {
    ...
    // Get total giveaway bubbles received for this category
    ...
    // Get total already used for offers in this category (we'll track with giveaway=1)
...
// Redeem offer using giveaway bubbles or personal bubbles
router.post('/redeem-offer', auth, async (req, res) => {
  ...
  try {
    ...
    // Get available giveaway bubbles for this category
    ...
    // Calculate how to cover the price
    if (availableGiveawayBubbles >= price) {
      // Can fully cover with giveaway bubbles
      usedGiveawayBubbles = price;
    } else {
      // Use all available giveaway bubbles
      usedGiveawayBubbles = availableGiveawayBubbles;
      const remaining = price - usedGiveawayBubbles;

      // Check personal bubbles
      if (user.bubblesCount >= remaining) {
        // Can cover remainder with personal bubbles
        usedPersonalBubbles = remaining;
      } else if (remaining - user.bubblesCount <= 100) {
        // Shortfall is 100 or less - needs admin approval
        usedPersonalBubbles = user.bubblesCount;
        shortfall = remaining - user.bubblesCount;
        needsAdminApproval = true;
      } else {
        // Shortfall is more than 100 - cannot proceed
        await t.rollback();
        return res.status(400).json({
          message: `Insufficient bubbles. You need ${remaining} more but only have ${user.bubblesCount} personal bubbles. Shortfall of ${remaining - user.bubblesCount} exceeds the 100 bubble limit.`,
    ...
    console.log(`Redemption breakdown: Giveaway=${usedGiveawayBubbles}, Personal=${usedPersonalBubbles}, Shortfall=${shortfall}`);
    ...
    // Create redemption transaction for giveaway bubbles
    if (usedGiveawayBubbles > 0) {
      await BubbleTransaction.create({
        fromUserId: userId,
        toUserId: brandId, // Brand receives the redemption
    ...
    // Create redemption transaction for personal bubbles
    if (usedPersonalBubbles > 0) {
      await BubbleTransaction.create({
        fromUserId: userId,
        toUserId: brandId,
    ...
    // If needs admin approval, create pending request
    if (needsAdminApproval) {
      await OfferRequest.create({
        ...
        scheduledTime: new Date().toTimeString().split(' ')[0],
        status: 'pending',
        adminNotes: `Shortfall: ${shortfall} bubbles. Used ${usedGiveawayBubbles} giveaway + ${usedPersonalBubbles} personal bubbles for ${price} price offer.`
      }, { transaction: t });
    }
...
// Get user's offer redemption history
router.get('/my-redemptions', auth, async (req, res) => {
  try {
    ...
    // Group by offer (extract offer ID from description)
